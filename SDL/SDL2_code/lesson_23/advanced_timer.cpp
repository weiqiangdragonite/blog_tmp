...
main(int argc, char *argv[])
{
	...
	while (quit == 0) {
		...
		snprintf(timeText, sizeof(timeText), "Seconds since start time %f s", timer.getTicks() / 1000.0f);
...
load_media(void)
{
	...
	if (gStartPromptTexture.loadFromRenderedText(
		"Please S to Start/Stop the Timer.", textColor) == -1)
	{
		fprintf(stderr, "Unable to start/stop prompt texture!\n");
		return -1;
	}

	/* load pause prompt texture */
	if (gPausePromptTexture.loadFromRenderedText(
		"Please p to Pause/Unpause the Timer.", textColor) == -1)
	{
		fprintf(stderr, "Unable to pause/unpause prompt texture!\n");
		return -1;
	}
