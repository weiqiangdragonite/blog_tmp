...
CODE_START:
        ...
        @ 设置栈，应为CPU会把NAND前4K复制到SRAM的Steppingstone内，所以设置
        @ 栈底为4K处，即0x00001000
        @MOV         SP, #4096

		@ 重定位代码
		ADR			R0, CODE_START					@ CODE_START地址为0
		LDR			R1, =CODE_START					@ CODE_START的连接地址
		LDR			R2, =bss_start					@ bss_start的开始地址
		CMP			R0, R1							@ 看CODE_START是否已经处于连接地址
		BEQ			CLEAN_BSS						@ 处于连接地址，跳到BSS段清零处
...
CLEAN_LOOP:
		...
		@ 这里不能用LDR跳到INIT_CLOCK的连接地址，要不然回不来
		@ 所以要先跳到START的连接地址，然后再跳去INIT_CLOCK
		LDR			PC, =START
START:
		@ 以下开始从连接地址处运行
		NOP
