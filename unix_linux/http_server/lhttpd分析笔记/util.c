...
{
	unsigned int loop=0;
	fgets(line, 255, in);            //获取一行
	for(loop=0; loop<255 && line[loop]!='\0'; loop++) 
	  if(line[loop]=='\r' || line[loop]=='\n')           //因为fgets遇到换行符号时会停止读取，并且会把换行符也拷贝到string中，所以这里替换为char*字符的终止符
	         line[loop] = '\0';
}
...
unsigned long count_vhosts()
{
	...
	if((in = fopen("/etc/lhttpd.conf", "rt")) == NULL)
		return 0;
		
	while(!feof(in))
	{
		memset(tempstring, 0, 255);   //每次都要清空！
		getfileline(tempstring, in);  //循环获取每一行的内容，函数定义在上文
		if(feof(in)) break;
		
		ptr1 = strtok(tempstring, " \t\n\r");  //将整串按照分割符来分割，只要有一个适配就好
		if(ptr1 && !gstricmp(ptr1, "<virtualhost>"))  //如果在conf中找到多个匹配的<virtualhost>，记录数目
			count++;
	}
...
void readinconfig()
{
	...
	while(!feof(in))
	{
		...
		if(!gstricmp(ptr1, "ServerRoot"))
		...
		else if(!gstricmp(ptr1, "DocumentRoot"))                     // virtualhost的配置项
		...
		else if(!gstricmp(ptr1, "ServerPort"))                       // 全局的配置项
			...
			VirtualHostDefinition(in);                               // 对多个virtual host的信息进行保存
		else
...
int gstricmp(char *string1, char *string2)
{
	...
	if(!string1 || !string2) return 1;         //任意一个串为空，返回1

	while(*string1 && *string2)
    	if(tolower(*(string1++)) != tolower(*(string2++))) return 1;     //循环对串的每个字符进行比较，忽略大小写，如果失配，返回1
