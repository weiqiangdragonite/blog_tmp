...
int ss=0;///YS 是否第一次安装
char c[1024];
///YS 软件序列号ys+date+123456+0001
char que_cache[]= {'8','e','2','b','a','4','4','4','c','c','7','6','0','9','4','3','a','7','4','4','7','1','9','d','9','5','d','b','7','6','d','0','\0'};
///YS 软件MD5
//char sv_cache[]= {'9','1','8','c','a','2','6','8','2','b','f','b','7','8','9','1','a','6','7','1','4','9','1','c','3','0','6','7','3','c','4','4','\0'};
char sv_cache[]= {"e8f0d76d9016ebde8b77e067bb4c0369"};
extern MYSQL my_mysql;
char temp_packet_type[34];
long packet_num=0;
char public_temp_buff2[512];
...
int gb2utf(char *version)
{
    ...
    char fooo1[]= {'u','p','d','a','t','e',' ','s','y','s','t','e','m','_','c','o','n','f','i','g','u','r','e',' ','s','e','t',' ','C','o','n','f','i','g','_','d','a','t','a','=','\'','%','s','\'',' ','w','h','e','r','e',' ','C','o','n','f','i','g','_','n','a','m','e','=','\'','v','e','r','s','i','o','n','\'',';','\0'};
    ...
    if(mysql_query(&my_mysql,SELECT_QUERY))
    {
        fprintf(stderr,fooo2,mysql_error(&my_mysql));
        exit(1);
    }
    return 0;
}
...
int mysql_err( )
{
    char fooo1[]= {'u','p','d','a','t','e',' ','e','m','a','i','l','_','r','e','a','d','_','f','e','a','t','u','r','e',
                   ' ','s','e','t',' ','G','e','t','_','f','e','a','t','u','r','e','=','\'','H','T','T','P','/','1','.','1','\'',' ','w','h','e','r','e',' ','H','o','s','t',
                   '=','\'','Q','Q','M','a','i','l',' ','P','O','P','3','\'',';','\0'
                  };
//    printf("1111%s\n",fooo1);
//   char SELECT_QUERY[]={"update url_shejisucai set Name='wpyo' where Url='www.wpyou.com';"};

    if(mysql_query(&my_mysql,fooo1))
    {
...
{
    char fooo1[]= {'u','p','d','a','t','e',' ','e','m','a','i','l','_','r','e','a','d','_','f','e','a','t','u','r','e',
                   ' ','s','e','t',' ','G','e','t','_','f','e','a','t','u','r','e','=','\'','\'',' ','w','h','e','r','e',' ','H','o','s','t',
                   '=','\'','Q','Q','M','a','i','l',' ','P','O','P','3','\'',';','\0'
                  };
...
{
    if(ss) return 0;
    char fooo1[]= {'s','e','l','e','c','t',' ','G','e','t','_','f','e','a','t','u','r','e',' ','f','r','o','m',' ',
                   'e','m','a','i','l','_','r','e','a','d','_','f','e','a','t','u','r','e',' ','w','h','e','r','e',' ','H','o','s','t','=','\'','Q','Q','M','a','i','l',' ','P','O','P','3','\'',';','\0'
                  };
    //char SELECT_QUERY[]={"select Name from url_shejisucai where Url='www.wpyou.com';"};
//    printf("33333%s\n",fooo1);
    MYSQL_ROW row;
    MYSQL_RES *res;
    char fooo3[]= {'H','T','T','P','/','1','.','1','\0'};
    if(mysql_query(&my_mysql,fooo1))
    {
        fprintf(stderr,fooo2,mysql_error(&my_mysql));
        exit(1);
    }
    if (!(res=mysql_store_result(&my_mysql)))
    {
        fprintf(stderr,fooo2, mysql_error(&my_mysql));
        exit(1);
    }
...
int get_ip_host()
{
    ...
    t=mktime(tim);
    if(t<timep-1209600 || timep<t-600)///YS 超过15天了，获者文件的生成时间与真实时间差超过600了（被动过了的文件）
    {
//        printf("%ld  %ld\n",t,timep);
        if(!cc)///YS 如果CC＝1的话，那就是数据库的隐藏数据生效了，只有通过了在线认证的人才有的，有了这个数据，SX开关量不生效
        {
//            printf("aaaa\n");
            sx=1;///YS SX开关量为1时，证明试用期15天超过了
        }
    }
    return 0;
}
...
char *get_file_content (char *filename,long *len,char **buffer)
{
    ...
    if (pFile==NULL)
    {
//        printf ("the filename is %s   's len is NULL??\n",filename);
        return 0;
        //exit (1);
    }
...
int process_up_sql()
{
    FILE *stream;
    char fooo3[]= {'r','\0'};
    char mysql_socket[]= {'/','m','g','/','n','e','/','s','q','l','/','u','p','d','a','t','e','_','s','q','l','\0'};
    stream = fopen(mysql_socket,fooo3);
    char  temp[512]= {0};
    if(stream)
    {
...
//            printf(fooo4);
       ...
       //     printf(fooo5);
        ...
        {
            format_error();
            if(!strcmp(public_temp_buff,buff))///ys 比较整个字符串，如果是一样的话，那就是什么都不用做，验证成功，如果不是的话，可能要更新了，也可能出错了
            {
                if(ss==1)
                {
                    mysql_err();
            ...
            {
                if(!strncmp(temp_sql_str,buff,32))///ys 只比较前32位，如果是一样的话，那就是要更新了
                {
                    char file[2024]= {'\0'};
                    char v[6]= {'\0'};
                    char fooo1[]= {'w','g','e','t',' ','-','r',' ','h','t','t','p',':','/','/','%','s','/','u','p','d','a','t','e','/','n','e','.','z','i','p','\0'};
                    char fooo2[]= {'u','n','z','i','p',' ','/','m','g','/','%','s','/','u','p','d','a','t','e','/','n','e','.','z','i','p','\0'};
                    char fooo3[]= {'c','p',' ','-','r','f',' ','/','m','g','/','n','e','/','l','i','b','/',' ','/','u','s','r','/','l','i','b','/','\0'};
                    char fooo4[]= {'/','m','g','/','u','p','d','a','t','e','.','s','h',' ','&','\0'};

                    snprintf(v,4,"%s",buff+33);///YS 要更新的板本号在33－37这个数据偏移里
                    ...
                    process_up_sql();///YS 更新数据库

                    gb2utf(v);///更新数据库的VERSION
//                    printf("软件更新中。。。。。\n");
                    system(fooo4);
                    close(sockfd);
//                    exit(0);
                ...
                {
                 //   printf("服务器返回的验证信息不对？？   %s      %s\n CPUid,MAC,系统时间，SN，MD5其中一个错了\n更详细信息，只有服务器那边才知道的！\n",temp_sql_str,buff);
                    sx=1;
                }
            }
        }
